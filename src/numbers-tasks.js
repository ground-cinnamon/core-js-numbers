...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const xy1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const xy2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const multiple = xy1 * xy2;
  const angleRadians = Math.acos(sum / multiple);
  return angleRadians;
}
...
function toPrecision(number, precision) {
  const formattedNumber = parseFloat(number.toPrecision(precision)).toString();
